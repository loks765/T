let handler = async (m, { conn, text, usedPrefix, command, args, participants, isOwner }) => {
  ...
  if (!isOwner) return conn.sendMessage(m.chat,{text:`*دعوة بوت إلى مجموعة*\n\nمرحبًا @${m.sender.split('@')[0]}\nيمكنك استئجار الروبوت للانضمام إلى مجموعة\n\n_لمزيد من المعلومات يمكنك مراسلة المالك_\n*يكتب* \`\`\`.مالك\`\`\` *إلى المالك مباشرة*`.trim()}, {quoted:m});
...
@${conn.user.jid.split('@')[0]} سيغادر تلقائيًا بعد \n\n${msToDate(global.db.data.chats[res].expired - now)}`
 await conn.sendButton(res, mes, null, [[`✆ Owner`, `${usedPrefix}fgowner`], [`⦙☰ Menu`, `${usedPrefix}help`]], m, {
       mentions: d
        })
    })
   } catch (e) {
